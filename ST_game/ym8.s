; probleme lors du bouclage ??

; - a voir par la suite , decompression 2: delta encoding + LZ4 compression

; YM8 sur Jaguar
;
; TODO done :
; 	OK - malloc ram centrale pour STF samples ( 256 maxi)
; 	OK - recalculer YM_table_frequences_Sinus_Sid_Amiga
; 	OK - reecrire code enveloppe  ;	- une tables des enveloppes  ;	- une enveloppe = liste de 1 etape + 2 etapes. 16 valeurs par etape => 48 valeurs ;	- bouclage sur etape 1 = valeur 16*4
;	OK - verifier le bouclage et le dernier bloc avec la nouvelle table LZ4
; 	OK - verifier generateur de noise
;	OK - relire toute la doc YM8 et verifier qu'elle est bien appliquée
;	OK - revoir table de volumes... : 62,161,265,377,580,774,1155,1575,2260,3088,4570,6233,9330,13187,21220,32767
;	OK - voir pour creer un maximum de choses en RAM DSP
...
;YM_Samples_STE_en_RAM_DSP			.equ		1						; 0 = samples STE en RAM 68000 / 1 = samples STE en RAM DSP.
...
; apres copie on init le YM7
	bsr			YM_init_ym8__collecte_des_variables				; OK
	bsr			YM_init_ym8__creation_des_tables					; OK
	bsr			YM_init_ym8_affichage_infos							; OK
	bsr			YM_init_YM8__decompression_LZ4						; OK
...
init_YM8__sortie_erreur_pas_YM8:
		lea		texte_erreur_pas_YM8,a0
		bsr		print_string		
		rts

init_YM8__sortie_erreur_depacker_pas_supporte:
		lea		texte_erreur_depacker_pas_supporte,a0
		bsr		print_string		
		rts

init_YM8__sortie_erreur_pas_packe:
		lea		texte_stored,a0
		bsr		print_string		
		rts
	
YM_init_ym8__collecte_des_variables:
; init de variables et de tables

	lea			fichier_ym8,A0
	; check 'YM8!'

		cmp.b	#'Y',(a0)+
		bne.s	init_YM8__sortie_erreur_pas_YM8
		cmp.b	#'M',(a0)+
		bne.s	init_YM8__sortie_erreur_pas_YM8
		cmp.b	#'8',(a0)+
		bne.s	init_YM8__sortie_erreur_pas_YM8
...
; calcul des pointeurs sur les samples

		move.l		a0,a2

; enrichie les variables		
		move.l		20-12(a2),YM_nombre_de_frames_totales
		moveq		#0,d0
		move.w		24-12(a2),d0
		move.l		d0,YM_frequence_replay
		move.w		26-12(a2),d0
		move.l		d0,YM_flag_effets_sur_les_voies
		
		lea			2+4+2+4+2+2(a2),a2
		moveq		#0,d0
		move.w		(a2)+,d0				; SID wavelets count in the file
		move.w		d0,YM_SID_wavelets_count
		move.l		a2,YM_pointeur_sur_la_liste_des_tailles_des_samples_SID
		moveq		#0,d1					; D1 = taille totales des samples SID
		cmp.w		#0,d0
		beq.s		init_YM8_boucle1__sortie
init_YM8_boucle1:
		add.w		(a2)+,d1
		subq.w		#1,d0
		bne.s		init_YM8_boucle1
init_YM8_boucle1__sortie:
		move.l		d1,YM_taille_totale_samples_SID
		
		moveq		#0,d0
		move.w		(a2)+,d0				; SyncBuzzer wavelets count in the file
		move.w		d0,YM_SyncBuzzer_wavelets_count
		move.l		a2,YM_pointeur_sur_la_liste_des_tailles_des_samples_SyncBuzzer
		moveq		#0,d1					; D1 = taille totales des samples SyncBuzzer
		cmp.w		#0,d0
		beq.s		init_YM8_boucle2__sortie
init_YM8_boucle2:
		add.w		(a2)+,d1
		subq.w		#1,d0
		bne.s		init_YM8_boucle2
init_YM8_boucle2__sortie:
		move.l		d1,YM_taille_totale_samples_SyncBuzzer

		moveq		#0,d0
		move.w		(a2)+,d0				; STF sample count
		move.w		d0,YM_STF_samples_count
		move.l		a2,YM_pointeur_sur_la_liste_des_tailles_des_samples_STF
		moveq		#0,d1					; D1 = taille totales des samples STF
		cmp.w		#0,d0
		beq.s		init_YM8_boucle3__sortie
init_YM8_boucle3:
		add.w		(a2)+,d1
		subq.w		#1,d0
		bne.s		init_YM8_boucle3
init_YM8_boucle3__sortie:
		move.l		d1,YM_taille_totale_samples_STF

		moveq		#0,d0
		move.w		(a2)+,d0				; STE sample count
		move.w		d0,YM_STE_samples_count
		move.l		a2,YM_pointeur_sur_la_liste_des_tailles_des_samples_STE
		moveq		#0,d1					; D1 = taille totales des samples STE
		cmp.w		#0,d0
		beq.s		init_YM8_boucle4__sortie
init_YM8_boucle4:
		add.w		(a2)+,d1
		subq.w		#1,d0
		bne.s		init_YM8_boucle4
init_YM8_boucle4__sortie:
		move.l		d1,YM_taille_totale_samples_STE

		move.w		(a2),YM_byte_streams_count
		move.w		(a2)+,YM_nb_registres_par_frame		; .w

; minimal memory buffer size to decode one YM block, in KiB
		moveq		#0,d0
		move.w		(a2)+,d0
		mulu		#1024,d0
		move.l		d0,YM_minimal_memory_buffer_size			; .l
		
; YM blocks count in the file
		move.w		(a2)+,d0			; YM blocks count in the file
		ext.l		d0
		move.l		d0,YM_nombre_de_blocs_lZ4
		
; YM blocks packing method
		move.w		(a2)+,d7			; YM blocks packing method
		cmp.w		#2,d7
		beq			init_YM8__sortie_erreur_depacker_pas_supporte
		cmp.w		#1,d7
		bne			init_YM8__sortie_erreur_pas_packe
	
		move.l		a2,YM_pointeur_sur_le_debut_des_tailles_des_paquets_LZ4

; N*
; number of frames in this block .L
; packed size of the block .L

		
		lsl.l		#3,d0					; YM blocks count in the file * ( number of frames in this block.L + packed size of the block.L )
		add.l		d0,a2
		
		lea			2(a2),a2				; bytes to skip ( made for future extension. Generally “0”)
		
		move.l		a2,YM_pointeur_Music_title_name
		
		moveq		#0,d0
		move.b		(a2)+,d0				; Music title name size
		add.l		d0,a2

		move.l		a2,YM_pointeur_Music_composer


		moveq		#0,d0
		move.b		(a2)+,d0				; Music composer size
		add.l		d0,a2

		move.l		a2,YM_pointeur_Music_comment

		moveq		#0,d0
		move.b		(a2)+,d0				; Music comment size
		add.l		d0,a2
		
		move.l		a2,YM_pointeur_sur_le_debut_des_samples_SID
		add.l		YM_taille_totale_samples_SID,a2
		move.l		a2,YM_pointeur_sur_le_debut_des_samples_SyncBuzzer
		add.l		YM_taille_totale_samples_SyncBuzzer,a2
		move.l		a2,YM_pointeur_sur_le_debut_des_samples_STF
		add.l		YM_taille_totale_samples_STF,a2
		move.l		a2,YM_pointeur_sur_le_debut_des_samples_STE
		add.l		YM_taille_totale_samples_STE,a2
		move.l		a2,YM_pointeur_sur_le_debut_des_paquets_LZ4
	
		rts

; ------------------ ---------- ---------
YM_init_ym8__creation_des_tables:
	
	move.w		YM_STF_samples_count,d7
	ext.l		d7
...
;	- multiplicateur / nb de fois = 1 pour digidrums
	...
	bsr			YM_malloc_DSP
	cmp.l		#-1,d0
	bne.s		YM_init_alloc_DSP1_OK_DG
	bsr			YM_malloc
YM_init_alloc_DSP1_OK_DG:
	move.l		d0,a2
	move.l		a2,pointeur_sur_YM_DSP_table_digidrums
	;move.l		d7,d6					; D6=nb samples DG
	;lsl.l		#2,d6					; * 4 octets 
	move.l		YM_pointeur_sur_le_debut_des_samples_STF,a1			; A1 = pointe sur debut samples DG
	move.l		YM_pointeur_sur_la_liste_des_tailles_des_samples_STF,a0
...
YM_init_ym7_boucle_table_digidrums:
	...
	move.l		d2,(a2)+					; stocke le pointeur sur le sample DG dans la table, pré shifté de 11 bits
	moveq		#0,d0
	...
	lsl.l		#8,d2						; pré shiftés de 11 bits
	lsl.l		#3,d2						; pré shiftés de 11 bits
	move.l		d2,(a2)+					; stocke l'adresse de la fin du sample DG dans la table , pré shifté de 11 bits
...
; creation tables SID
	move.w		YM_SID_wavelets_count,d7
	ext.l		d7
	cmp			#0,d7
...
; d7=nb de samples SID
	...
	move.l		YM_pointeur_sur_la_liste_des_tailles_des_samples_SID,a0
...
; * 4 pour la RAM DSP
	...
	move.l		YM_pointeur_sur_le_debut_des_samples_SID,a0
	...
	bsr			YM_malloc_DSP
	cmp.l		#-1,d0
	bne.s		YM_init_alloc_DSP1_OK
	bsr			YM_malloc
YM_init_alloc_DSP1_OK:
	...
	bsr			YM_malloc_DSP
	cmp.l		#-1,d0
	bne.s		YM_init_alloc_DSP2_OK
	bsr			YM_malloc
YM_init_alloc_DSP2_OK:
...
;---------------- ------------ ----------------
; creation tables syncbuzzer

	move.w		YM_SyncBuzzer_wavelets_count,d7
	ext.l		d7
	cmp			#0,d7
	beq			YM_pas_de_Buzzer
...
; - une table de .L : ( pointeur sur enveloppes buzzer, taille de l'enveloppe buzzer ) * nb env buzzer : 2*.L
	...
	bsr			YM_malloc_DSP
	cmp.l		#-1,d0
	bne.s		YM_init_alloc_DSP3_OK
	bsr			YM_malloc
YM_init_alloc_DSP3_OK:
	move.l		d0,a4
...
; - une table de .L : pour chaque env-buzzer : pointeurs sur enveloppes * taille de l'env-buzzer
	...
	move.l		YM_pointeur_sur_la_liste_des_tailles_des_samples_SyncBuzzer,a0

YM_init_ym7_calcul_taille_table_pointeurs_env_buzzer:
	moveq		#0,d2
	move.w		(a0)+,d2		; taille d'une env-buzzer
	add.l		d2,d1
	...
	bsr			YM_malloc_DSP
	cmp.l		#-1,d0
	bne.s		YM_init_alloc_DSP4_OK
	bsr			YM_malloc
YM_init_alloc_DSP4_OK:
	move.l		d7,d6		;D6=nb env-buzzer
	subq.l		#1,d6		; -1 pour dbf

	move.l		d0,a1		; A1=pointeur sur liste des env des env-buzzer

	lea			YM_DSP_liste_des_enveloppes_V2,a2
	move.l		YM_pointeur_sur_la_liste_des_tailles_des_samples_SyncBuzzer,a0
	move.l		YM_pointeur_sur_le_debut_des_samples_SyncBuzzer,a3

YM_init_ym7_boucle_remplit_table_pointeurs_sur_env_buzzer:
	moveq		#0,d2
	move.w		(a0)+,d2	; D2=taille de l'env-buzzer
...
; -------------------------------------------------
; creation tables Sinus sid / STE samples

	move.w		YM_STE_samples_count,d7
	ext.l		d7
	cmp			#0,d7
...
;		- copier les octets de mono au debut du Sinus Sid / supprimer la stereo
	...
	bsr			YM_malloc_DSP
	cmp.l		#-1,d0
	bne.s		YM_init_alloc_DSP5_OK
	bsr			YM_malloc
YM_init_alloc_DSP5_OK:
; d0 = pointeur sur la tables des infos des samples des sinus sid
	move.l		d0,YM_DSP_pointeur_table_samples_Sinus_Sid
	move.l		d0,a2

	move.l		YM_pointeur_sur_la_liste_des_tailles_des_samples_STE,a0
	move.l		YM_pointeur_sur_le_debut_des_samples_STE,a1			; A1 = pointe sur les samples sinus sid
...
YM_init_YM8__decompression_LZ4:
; A0=All packed YM blocks

; créer la table listant les blocs
; 	- debut du bloc .L
;	- nombre de frames dans le bloc ( taille du bloc decompressé = nb de frames * YM_byte_streams_count )  .L
;	- taille du bloc compressé	.L

	moveq		#0,d0
	move.l		YM_nombre_de_blocs_lZ4,d0
	move.l		d0,d7
	mulu		#12,d0
	bsr			YM_malloc_DSP
	cmp.l		#-1,d0
	bne.s		YM_init_alloc_DSP6_OK
	bsr			YM_malloc
YM_init_alloc_DSP6_OK:

	move.l		d0,YM_DSP_pointeur_sur_table_infos_blocs_LZ4
	
	move.l		YM_pointeur_sur_infos_blocs_LZ4,a2

	move.l		d0,a3
	subq.w		#1,d7
	
	move.l		YM_pointeur_sur_le_debut_des_paquets_LZ4,a0
	move.l		YM_pointeur_sur_le_debut_des_tailles_des_paquets_LZ4,a2
	
YM8_init_boucle_pointeur_sur_blocs_LZ4:
	move.l		a0,(a3)+			; debut du bloc .L
	move.l		(a2)+,(a3)+			; number of frames in this block .L
	move.l		(a2)+,d1
	move.l		d1,(a3)+			; packed size of the block
	add.l		d1,a0
	dbf			d7,YM8_init_boucle_pointeur_sur_blocs_LZ4
...
; decompression 1er bloc
; allouer la RAM
	move.l		YM_minimal_memory_buffer_size,d0
; d0=taille à allouer
	bsr			YM_malloc
; d0=pointeur sur la zone memoire vide
	...
	move.l		a1,a3
	move.l		YM_DSP_pointeur_sur_table_infos_blocs_LZ4,a2
	move.l		(a2),a0		; premier bloc LZ4
	move.l		4(a2),d4	; nombre de frames dans le bloc
	move.l		8(a2),d0	; taille du bloc compressé	.L
...
; check longueur ok
	...
	move.l		a1,d1
	sub.l		a3,d1			; D1 = taille decompressée

	moveq		#0,d2
	move.w		YM_byte_streams_count,d2
	mulu		d4,d2			; D2 = taille calculée decompressée

	cmp.l		d1,d2
	beq.s		YM8_init__LZ4_decomp_bloc1_OK
	nop
	
YM8_init__LZ4_decomp_bloc1_OK:


; decompression 2eme bloc
; allouer la RAM
	move.l		YM_minimal_memory_buffer_size,d0
; d0=taille à allouer
	bsr			YM_malloc
; d0=pointeur sur la zone memoire vide
	...
	; EDZ : commencer par le bloc 2
	; move.l		d0,YM_LZ4_pointeur_bloc_LZ4_en_cours

	
	
	move.l		YM_nombre_de_blocs_lZ4,d2
	subq.w		#1,d2
	bgt.s		YM8_init__LZ4_decomp_bloc2_a_faire

; il n'y a q'un seul bloc, il faut juste le copier dans le 2eme buffer

	move.l		YM_minimal_memory_buffer_size,d6
	lsr.l		#2,d6
	subq.l		#1,d6
	move.l		YM_LZ4_pointeur_bloc_LZ4_en_cours,a1
	move.l		YM_LZ4_pointeur_bloc_LZ4_suivant,a2

YM8_init__LZ4_decomp_bloc2_boucle_copie:	
	move.l		(a1)+,(a2)+
	dbf			d6,YM8_init__LZ4_decomp_bloc2_boucle_copie

; FIN quand 1 seul bloc
; on est prêt à parcourir les registres YM
	move.l		YM_DSP_pointeur_sur_table_infos_blocs_LZ4,a2
	;move.l		(a2),a0		; premier bloc LZ4
	move.l		4(a2),d4	; nombre de frames dans le bloc
	;move.l		8(a2),d0	; taille du bloc compressé	.L
	...
	;add.l		#YM_position_debut_dans_musique,a0
	move.l		a0,YM_pointeur_actuel_ymdata
	move.l		d4,PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_en_cours
	;sub.l		#YM_position_debut_dans_musique,d4
	move.l		d4,PSG_compteur_frames_restantes

	;EDZ : Avancer juste avant le bloc 2
	;move.l		#4096,PSG_compteur_frames_restantes
	

; infos bloc 2 = info bloc 1
	move.l		d4,PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_suivant


	move.l		#2,YM_LZ4_nb_bloc_LZ4_disponibles
	move.l		#0,YM_numero_bloc_LZ4_suivant_a_decompresser			; le prochain bloc a decompresser est le 3-1 ( 0 1 2...)
...
YM8_init__LZ4_decomp_bloc2_a_faire:

	move.l		d0,a1		; = dest de le decompression
	move.l		a1,a3
	move.l		YM_DSP_pointeur_sur_table_infos_blocs_LZ4,a2
	lea			12(a2),a2		; on passe au deuxieme bloc
	move.l		(a2),a0		; 2eme bloc LZ4
	move.l		4(a2),d4	; nombre de frames dans le bloc
	move.l		8(a2),d0	; taille du bloc compressé	.L
...
; check longueur ok
	...
	cmp.l		d1,d2
	beq.s		YM8_init__LZ4_decomp_bloc2_OK
	nop
	

YM8_init__LZ4_decomp_bloc2_OK:
; on est prêt à parcourir les registres YM
	move.l		YM_DSP_pointeur_sur_table_infos_blocs_LZ4,a2
	;move.l		(a2),a0		; premier bloc LZ4
	move.l		4(a2),d4	; nombre de frames dans le bloc
...
; infos bloc 2
	move.l		16(a2),PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_suivant


	move.l		#2,YM_LZ4_nb_bloc_LZ4_disponibles
	move.l		#3-1,YM_numero_bloc_LZ4_suivant_a_decompresser			; le prochain bloc a decompresser est le 3-1 ( 0 1 2...)

; -------- fin de decompression des 2 premiers blocs LZ4
...
;----------------------------------------------------
YM_init_ym8_affichage_infos:
; ne fait que afficher les infos
;----------------------------------------------------

	move.b		#225,couleur_char

; affiche YM8
	lea			chaine_debut_init_YM8,a0
	bsr			print_string

	move.b		#181,couleur_char


		move.l		YM_pointeur_Music_title_name,a0
		; Music title name
		move.l		a0,a2
		lea			texte_title,a0
		bsr			print_string
		...
		add.l		d0,a1
		addq.l		#1,a1
		move.b		#171,couleur_char
		bsr			print_string__sstring
		moveq		#10,d0				; saut de ligne
		bsr			print_caractere
		move.l		a1,a0
		
;  Music title name
		move.b		#151,couleur_char
		move.l		a0,a2
		lea			texte_composer,a0
		bsr			print_string
		move.l		a2,a0
		moveq		#0,d0
		move.b		(a0),d0
...
;  YM8 sub-format version
		lea			fichier_ym8+12,a0
		move.l		a0,a1
		lea			texte_YM8_sub_format_version,a0
		bsr			print_string
		move.l		a1,a0
		moveq		#0,d0
		move.w		(a0)+,d0
...
; tool producing the YM file + version
		move.l		a0,a1
		lea			texte_production_tool,a0
		bsr			print_string
		move.l		a1,a0
		moveq		#0,d0
		.rept		4
		...
		.endr
		move.l		a0,a1
		lea			texte_production_version,a0
		bsr			print_string
		move.l		a1,a0
		moveq		#0,d0
		move.b		(a0)+,d0
...
; affiche frequence de replay
	move.l		YM_frequence_replay,d0							; .w=frequence du replay ( 50 hz ?)
	move.l		d0,-(sp)
...
; affichage des effets
	move.l		YM_flag_effets_sur_les_voies,d0

; texte effets sur voie A
	cmp.l		#0,d0
...
; texte effets sample STE
	btst		#4,d0						; effet D Sinus Sid => sample STE
	beq.s		YM_init_ym7_pas_effet_Sinus_Sid
	move.l		a0,-(sp)
	lea			chaine_effet_STE_samples_init_YM7,a0
	bsr			print_string
	move.l		(sp)+,a0
...
; affiche nb DG / STF samples
	moveq		#0,d7
	move.w		YM_STF_samples_count,d7								; .w=nombre de digidrums
	move.l		a0,-(sp)
...
; debut init SID 
	moveq		#0,d7
	move.w		YM_SID_wavelets_count,d7								; .w= nombre de SID à fabriquer
	move.l		a0,-(sp)
...
; debut init 	Buzzer
	moveq		#0,d7
	move.w		YM_SyncBuzzer_wavelets_count,d7									; valeur dans le fichier YM7 = nombre de buzzer ( apres entete, digidrums et table SID )
...
; affiche init sinus sid
	moveq		#0,d7
	move.w		YM_STE_samples_count,d7									; D7=nb samples sinus sid / STE samples
	move.l		a0,-(sp)
...
; nb blocs LZ4:
	moveq		#0,d0
	move.l		YM_nombre_de_blocs_lZ4,d0										; nombre de bloc compressés
	move.l		d0,-(sp)
	bsr			print_nombre_2_chiffres
	move.l		(sp)+,d0
...
; on a un Buzzer
	...
	moveq	#0,R25					; simplification ENV : offset de 0 a 47
...
YM_DSP_replay_Buzzer_pas_de_bouclage_envbuzzer:
	...
	shrq	#16,R21					; 16 bits du haut de R19 = partie entiere de l'offset
	shlq	#2,R21					; *4 pour pointeur sur env
	add		R21,R23
...
; -------------------------------------------------------------------------------
	...
	movei		#PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_en_cours,R3
	load		(R0),R1
	load		(R3),R8
...
;--------------------------------
	...
	movei		#pointeur_sur_YM_DSP_table_digidrums,R3
	movei		#YM_DSP_registre8,R6
	load		(R3),R3
	load		(R6),R6
...
; on teste si on a un SID
	...
	jump		eq,(R4)					; %00 = 00: nothing
	nop									
										; %01 = 01: SID voice is currently playing
; routine SID
; il faut remplir :
...
; utilisés : R1/R2/R3/R5/R6/R7/R8/R9
	btst		#5,R2						; bit 13->5 : if set, you should reset the timer phase (ie restart timer). 
	jr			ne,DSP_lecture_registre_effet_Buzzer_clear_offset_envbuzzer_partie_entiere
	nop
	
; if set, you should reset the timer phase (ie restart timer). 
...
; remet a zero l'offset partie entiere de lecture de env buzzer
	...
	and			R10,R7					; mets à zero la partie haute de l'offset envbuzzer 16:16
	
DSP_lecture_registre_effet_Buzzer_PAS_de_clear_offset_envbuzzer_partie_entiere:
; clear partie virgule
	not			R10
	and			R10,R7
...
; -----------------------------
; gestion effet Sinus Sid / STE sample
...
; on doit gérer un sinus Sid
	...
	;add			R8,R1			; plus besoin d'avancer on a lu tous les octets

	;btst		#7,R2
	cmpq		#-1,R2			; If this byte is 0xff then it means that nothing is happening this frame (no STE sample trigger)
	jump		eq,(R4)
	;nop
	move		R2,R3
	moveq		#%11,R4
	...
	movei		#%111111000,R3				; mask pour ne garder que le numero de sample sur bits 2 à 7 << 1
	addq		#4,R6						; on passe à YM_DSP_pointeur_sample_Sinus_SID
	add			R2,R2						; *2, deja * 4 => donne * 8
...
; avancer dans la lecture du YM7 a gérer !!	
	...
	load		(R2),R3
	movei		#DSP_lecture_registres_player_VBL_YM8_pas_fin_du_bloc,R8
	subq		#1,R3
	
	;cmp			#0,R3
	jump		ne,(R8)
	nop
; fin du bloc LZ4 en cours
...
; swap le nb de frames dans les blocs
	addq		#4,R4		; bascule sur  PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_en_cours
	addq		#4,R5		; bascule sur  PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_suivant
	load		(R4),R6										; passe suivant dans en cours, et en cours dans suivant pour remplissage
	load		(R5),R3
	store		R3,(R4)
...
; decremente le nb de blocs dispos
	movei		#YM_LZ4_nb_bloc_LZ4_disponibles,R7			
	;store		R3,(R2)				; PSG_compteur_frames_restantes
	load		(R7),R6
	subq		#1,R6
	store		R6,(R7)

DSP_lecture_registres_player_VBL_YM8_pas_fin_du_bloc:
...
; variables replay
	movei	#YM_DSP_pointeur_sur_table_infos_blocs_LZ4,R1
	movei	#PSG_compteur_frames_restantes,R4
	load	(R1),R2
	addq	#4,R2
	load	(R2),R3			; R3 = nombre de frames dans le bloc
	or		R3,R3
	store	R3,(R4)
...
; il faut remplir PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_suivant
	movei	#YM_DSP_pointeur_sur_table_infos_blocs_LZ4,R20
	movei	#YM_numero_bloc_LZ4_suivant_a_decompresser,R21
	moveq	#12,R3
	load	(R21),R8				; R8 =YM_numero_bloc_LZ4_suivant_a_decompresser 
	movei	#PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_suivant,R22
	move	R8,R4
	load	(R20),R14				; R14 = pointeur sur les infos de blocs LZ4 ( indexé )
	mult	R3,R4					; R4 = YM_numero_bloc_LZ4_suivant_a_decompresser * 12
	addq	#1,R8					; YM_numero_bloc_LZ4_suivant_a_decompresser + 1
	add		R4,R14					; positionner sur le bloc a decompresser
	
	load	(R14),R20				; R20 = adresse du bloc LZ4
	load	(R14+1),R11			; nombre de frames dans le bloc LZ4
	load	(R14+2),R0				; R0 = taille du bloc LZ4 compressé
; il faut MAJ PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_suivant
	store	R11,(R22)
	
	
; tester si dernier bloc dépassé
	movei	#YM_nombre_de_blocs_lZ4,R24
	load	(R24),R4
	cmp		R4,R8
...
; 
; registres utilisés :
; R0/R1/R2/R4/ /R10/R11/R12/ /R20/R21/R23/R24/R26
; registres disponibles :
; R3/R5/R6/R7/R8/R9
; R14/R15/R16/R17/R18/R19
; R22/R25/R27/R28/R29/R30
...
tokenLoop_smallest_DSP:
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack1:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack1
			nop
			.endif
...
readLoop_smallest1_DSP:
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack2:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack2
			nop
			.endif
...
litcopy_smallest_DSP:
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack3:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack3
			nop
			.endif
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack4:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack4
			nop
			.endif
...
lenOffset_smallest_DSP:
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack5:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack5
			nop
			.endif
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack6:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack6
			nop
			.endif
...
readLoop_smallest2_DSP:	
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack7:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack7
			nop
			.endif
...
copy_smallest_DSP:
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack9:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack9
			nop
			.endif
...
readLoop_smallest_DSP:	
			...
			.if		DSP_slow_down_depack>0
			movei	#DSP_slow_down_depack,R26
DSP_slow_down_depack10:
			nop
			subq	#1,R26
			jr		ne,DSP_slow_down_depack10
			nop
			.endif
...
; incremente le nb de blocs LZ4 disponibles
	movei	#YM_LZ4_nb_bloc_LZ4_disponibles,R0
	load	(R0),R1
	addq	#1,R1
	store	R1,(R0)
...
YM_DSP_pointeur_sur_table_infos_samples_SyncBuzzer:			dc.l		0				; SyncBuzzer wavelets data
YM_DSP_pointeur_sur_table_infos_samples_STF:					dc.l		0				; STF samples data
YM_DSP_pointeur_sur_table_infos_samples_STE:					dc.l		0				; STE samples data
YM_DSP_pointeur_sur_table_infos_blocs_LZ4:					dc.l		0				; liste blocs LZ4
...
; digidrums
; en memoire DSP
pointeur_sur_YM_DSP_table_digidrums:			dc.l		0
...
; LZ4:
YM_LZ4_nb_bloc_LZ4_disponibles:					dc.l			0
YM_numero_bloc_LZ4_suivant_a_decompresser:					dc.l		0
; YM8 LZ4
; do not move
YM_LZ4_pointeur_bloc_LZ4_en_cours:								dc.l			0
PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_en_cours:		dc.l			0
YM_LZ4_pointeur_bloc_LZ4_suivant:								dc.l			0
PSG_ecart_entre_les_registres_ymdata_bloc_LZ4_suivant:		dc.l			0
; do not move
YM_pointeur_sur_infos_blocs_LZ4:		dc.l					0
...
chaine_debut_init_YM8:			dc.b	"Init YM8",10,0
...
chaine_DG2_init_YM7:			dc.b	" STF samples.",10,0
chaine_SID1_init_YM7:			dc.b	"There are ",0
chaine_SID2_init_YM7:			dc.b	" SIDs.",10,0
chaine_effet_Buzzer_init_YM7:	dc.b	"SyncBuzzer in the YM8",10,0
chaine_effet_STE_samples_init_YM7:	dc.b	"STE samples in the YM8",10,0
...
chaine_Sinus_Sid2_init_YM7:			dc.b	" STE samples.",10,0
chaine_effet_voie_A_init_YM7:	dc.b	"SID fx or STF sample on channel A",10,0
chaine_effet_voie_B_init_YM7:	dc.b	"SID fx or STF sample on channel B",10,0
chaine_effet_voie_C_init_YM7:	dc.b	"SID fx or STF sample on channel C",10,0
...
texte_erreur_pas_YM8:					dc.b	"Error : File is no YM8",10,0
texte_erreur_depacker_pas_supporte:	dc.b	"Error : unsuported depacker.",10,0
texte_stored:								dc.b	"store (no compression)",10,0
texte_LZ4:									dc.b	"LZ4 compressed",10,0
texte_title:								dc.b	"Title : ",0
texte_composer:							dc.b	"Composer : ",0
texte_production_tool:					dc.b	"YM8 production tool : ",0
texte_production_version:				dc.b	" ver : ",0
texte_YM8_sub_format_version:			dc.b	"YM8 subformat : ",0
...
; 11: 50066 Hz
		...
		.dphrase
fichier_ym8:
			;.incbin		"C:/Jaguar/YM8/jess_ym8/sttigerf_64_lz4.ym8"
			;.incbin		"C:/Jaguar/YM8/5th gear1.ym8"				; bouclage ?    decompression/bruit ?
			;.incbin		"C:/Jaguar/YM8/end theme.ym8"					; OK
			; .incbin		"C:/Jaguar/YM8/st news 61.ym8"						; OK
			;.incbin		"C:/Jaguar/YM8/U_SUPSCR.YM8"					; BUG : bouclage/changement de bloc
			;.incbin		"C:/Jaguar/YM8/5th gear3.ym8"				; OK
			; .incbin		"C:/Jaguar/YM8/warp 1 intro.ym8"				; OK
			.incbin		"C:/Jaguar/YM8/cybernoid [cmp j.tel].ym8"
		.even
...
YM_SID_wavelets_count:					ds.w				1
YM_SyncBuzzer_wavelets_count:			ds.w				1
YM_STF_samples_count:					ds.w				1
YM_STE_samples_count:					ds.w				1

	.dphrase

YM_pointeur_sur_SID_wavelets:			ds.l				1
YM_pointeur_sur_SyncBuzzer_wavelets:			ds.l				1
YM_pointeur_sur_STF_samples:			ds.l				1
YM_pointeur_sur_STE_samples:			ds.l				1

YM_pointeur_sur_le_debut_des_samples_SID:				ds.l		1
YM_pointeur_sur_le_debut_des_samples_SyncBuzzer:		ds.l		1
YM_pointeur_sur_le_debut_des_samples_STF:				ds.l		1
YM_pointeur_sur_le_debut_des_samples_STE:				ds.l		1

YM_pointeur_sur_la_liste_des_tailles_des_samples_SID:			ds.l		1
YM_pointeur_sur_la_liste_des_tailles_des_samples_SyncBuzzer:	ds.l		1
YM_pointeur_sur_la_liste_des_tailles_des_samples_STF:			ds.l		1
YM_pointeur_sur_la_liste_des_tailles_des_samples_STE:			ds.l		1

YM_taille_totale_samples_SID:				ds.l		1
YM_taille_totale_samples_SyncBuzzer:		ds.l		1
YM_taille_totale_samples_STF:				ds.l		1
YM_taille_totale_samples_STE:				ds.l		1
YM_pointeur_sur_le_debut_des_tailles_des_paquets_LZ4:		ds.l		1
YM_pointeur_sur_le_debut_des_paquets_LZ4:						ds.l		1
YM_pointeur_Music_title_name:		ds.l		1
YM_pointeur_Music_composer:			ds.l		1
YM_pointeur_Music_comment:			ds.l		1
